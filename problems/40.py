# 실행: python 40.py < input.txt
"""
문제
살충제 2회 살포 (시뮬레이션 + 2D 누적합 + 비트마스크)
- N×N 격자에서 벌레들이 주어질 때, 살충제를 최대 두 번 살포해 제거할 수 있는 벌레의 최댓값을 구한다. 벌레는 방향과 속도를 갖고 이동하며, 경계를 만나면 반사된다.

입력
첫째 줄에 테스트 케이스 수 T가 주어진다.
각 테스트 케이스마다 다음 형식으로 입력이 주어진다.
- N M K
- 이후 K개의 줄에 r c d s 가 주어진다.

출력
각 테스트 케이스마다 최대 제거 가능한 벌레 수를 한 줄에 하나씩 출력한다.

제한
1 ≤ T ≤ 10 / 1 ≤ r, c ≤ N / 1 ≤ s ≤ 1000
...
해설
한 축의 움직임은 주기 cycle = 2×(N−1)을 갖는다. 시간 t에서의 위치는 초기 위치와 "방향 × (s × t mod cycle)" 만큼 이동한 결과를 반사 규칙으로 접어 계산할 수 있다. 주기가 cycle이므로 t = 0..cycle−1 구간만 조사해도 모든 상황을 포함한다. 각 시각별로 격자에 벌레 수를 세어 2차원 누적합으로 모든 M×M 구간의 벌레 수를 구하고, 상위 몇 개 구역만 후보로 뽑아 해당 구역에 포함되는 벌레 집합을 비트마스크로 저장한다. 이후 후보 쌍의 합집합 크기를 계산해 두 번의 살포로 제거할 수 있는 최대 수를 찾는다.
"""
