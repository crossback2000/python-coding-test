# 실행: python 31.py < input.txt
"""
문제
부분합이 제한된 가장 긴 구간 찾기 (슬라이딩 윈도우)
- N개의 비음수 정수와 합의 상한 S가 주어진다. 연속 부분배열 중 원소의 합이 S 이하인 가장 긴 길이를 구하여라. 조건을 만족하는 구간이 여러 개라면 길이만 출력하면 된다.
...
제한
1 ≤ N ≤ 200,000 / 0 ≤ S ≤ 10^12 / 0 ≤ a_i ≤ 10^9
...
해설
모든 숫자가 음이 아니므로, 왼쪽과 오른쪽 포인터를 움직이는 슬라이딩 윈도우로 현재 구간의 합을 유지하며 길이가 조건을 넘는 순간 왼쪽 포인터를 이동시켜 합을 줄인다. 모든 원소를 한 번씩만 이동시키므로 O(N) 시간에 답을 구할 수 있다.
