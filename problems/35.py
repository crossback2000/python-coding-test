...
"""
문제
로봇의 체크포인트 수거 (BFS + 비트마스크 DP)
- 격자 위에 로봇이 있으며, 특정 칸에 체크포인트가 있다. 로봇은 상하좌우로 이동할 수 있고 벽을 통과할 수 없다. 모든 체크포인트를 방문하는 데 필요한 최소 이동 횟수를 구하라.
...
해설
모든 체크포인트 사이의 최단 거리를 BFS로 미리 구해 그래프 형태로 변환한다. 이후 "모든 노드를 한 번씩 방문" 문제는 비트마스크 DP로 해결할 수 있다. 상태 dp[mask][i]는 현재 i번째 체크포인트에 있고 mask에 방문 기록이 있을 때의 최소 거리다.
