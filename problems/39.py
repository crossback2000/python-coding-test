...
"""
문제: 배열 분할 최소 비용 (구간 DP + 누적합)

길이 N의 배열을 정확히 K개의 연속 구간으로 분할하고자 한다. 각 구간의 비용은
구간 합과 구간 길이의 곱으로 정의한다. 전체 비용의 합이 최소가 되도록 분할했을 때의
최소 비용을 구하라.
...
해설 요약
구간 합이 자주 필요하므로 prefix sum으로 O(1)에 구간 합을 계산하고, dp[i][k]를
"앞에서 i번째 원소까지 k개의 구간으로 자를 때 최소 비용"으로 정의한다.
마지막 구간의 시작점을 j로 가정해 dp[i][k] = min(dp[j-1][k-1] + cost(j, i))를 갱신한다.
"""
