...
문제
2차원 구간합 질의 (누적합 전처리)
- N×N 격자에 정수가 채워져 있을 때, Q개의 직사각형 구간합 질의를 처리하라. 각 질의는 (r1, c1)에서 (r2, c2)까지의 좌상단/우하단 좌표가 주어지며, 해당 영역의 모든 원소 합을 출력해야 한다. 좌표는 1부터 시작한다.
...
해설
한 번의 전처리로 2차원 누적합을 구하면, 모든 구간합을 O(1) 시간에 답할 수 있다. 누적합 ps[r][c]는 (1,1)부터 (r,c)까지의 합을 의미하므로, 임의의 직사각형 합은 포함-배제 원리를 이용해 상수 시간에 계산된다.
