...
"""
문제: 실시간 구간 합 질의 (펜윅 트리)

N개의 정수 배열이 주어지고, 두 가지 연산을 처리해야 한다.
1) "1 idx val" : idx 위치의 값을 val로 갱신한다.
2) "2 l r" : 구간 [l, r]의 합을 출력한다.
연산은 총 Q번 주어지며, 1 ≤ idx, l, r ≤ N 이다.
...
해설 요약
배열이 크고 갱신과 질의가 번갈아 등장하므로, O(log N)에 갱신과 합 계산이 가능한 펜윅 트리(BIT)를 사용한다.
각 노드가 담당하는 구간 길이가 2의 거듭제곱이므로, 인덱스를 비트 연산으로 이동시키며 빠르게 합을 갱신/조회할 수 있다.
"""
