# 실행: python 38.py < input.txt
"""
문제
K번째 최단 경로 (우선순위 큐 확장 다익스트라)
- 가중치가 양수인 방향 그래프에서 시작점부터 모든 노드까지의 K번째 최단 경로 길이를 구한다. K번째 경로가 존재하지 않으면 -1을 출력한다.

입력
첫째 줄에 정점 수 N, 간선 수 M, 그리고 K가 주어진다.
이후 M개의 줄에 u v w 형태로 간선이 주어지며, 이는 u에서 v로 가중치 w의 간선이 있음을 의미한다.
마지막 줄에 시작 정점 S가 주어진다.

출력
각 정점에 대해 S에서 해당 정점까지의 K번째 최단 경로 길이를 1번 정점부터 한 줄에 하나씩 출력한다. 경로가 K개 미만이면 -1을 출력한다.

제한
1 ≤ N ≤ 2,000 / 0 ≤ M ≤ 50,000 / 1 ≤ K ≤ 100 / 가중치는 1 이상 10,000 이하
...
해설
기존 다익스트라는 한 노드당 최단 거리 하나만 관리하지만, 여기서는 최대 K개까지 거리를 저장한다. 우선순위 큐에서 거리를 꺼낼 때마다 해당 노드의 리스트에 추가하고, 아직 K개 미만일 때만 간선을 확장한다. 이렇게 하면 각 노드에서 최대 K번만 확장되므로 전체 시간은 O(K * M log (K*N)) 수준으로 동작한다.
"""
