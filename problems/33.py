...
문제
강의실 최소 개수 2 (우선순위 큐)
- 여러 강의의 시작 시각과 종료 시각이 주어질 때, 모든 강의를 겹치지 않게 배정하기 위해 필요한 최소 강의실 수를 구하라. 강의의 시작과 종료 시각은 정수이며, 종료 시각과 같은 시각에 다른 강의를 시작할 수 있다.
...
해설
강의를 시작 시각 기준으로 정렬한 뒤, 현재 진행 중인 강의들의 종료 시각을 최소 힙에 저장한다. 새 강의의 시작 시각보다 먼저 끝나는 강의는 힙에서 제거하여 같은 강의실을 재사용하고, 힙의 크기가 동시에 필요한 강의실 수가 된다.
