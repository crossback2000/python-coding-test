...
"""
문제: DAG에서 가장 긴 경로 (위상 정렬 + DP)

방향성이 있는 비순환 그래프(DAG)가 주어질 때, 가장 긴 경로의 길이를 구하라.
노드 수 N과 간선 수 M은 최대 200,000까지 가능하다. 경로 길이는 방문한 간선의 수로 정의한다.
...
해설 요약
DAG에서는 위상 정렬 순서대로 노드를 순회하면 모든 간선이 앞으로만 향한다.
각 노드의 최장 경로 길이를 dp로 저장하며 위상 순서대로 업데이트하면 전체 최장 경로를 얻을 수 있다.
"""
