...
문제
최소 회문 분할 (DP + 전처리)
- 문자열을 여러 조각으로 나누어 각 조각이 회문이 되도록 할 때, 필요한 최소 분할 횟수를 구하라. 분할 횟수는 조각의 개수 - 1 로 정의한다. 빈 문자열은 주어지지 않는다.
...
해설
회문 여부를 빠르게 확인하기 위해 모든 구간 [i, j]가 회문인지 O(L^2) 전처리한다. 이후 dp[i]를 0..i 구간을 회문들로 덮을 때 필요한 최소 분할 횟수로 정의하면, 최종 답은 dp[L-1]이다. 각 위치에서 가능한 회문 시작점을 확인하며 최솟값을 갱신한다.
